...
export function Navbar() {
  ...
  return (
    <header
      className={`fixed top-0 w-full flex justify-between items-center px-[4%] h-[70px] z-[1000] transition-colors duration-300 ${
        isScrolled ? 'bg-[#141414]' : 'bg-gradient-to-b from-black/80 to-transparent'
      }`}
    >
      <div className="flex items-center gap-5">
        <img
          className="w-[92px] mr-6 cursor-pointer"
          src="https://upload.wikimedia.org/wikipedia/commons/0/08/Netflix_2015_logo.svg"
          alt="Netflix"
        />
        <nav>
          <ul className="flex gap-5 text-[13px] list-none">
            {NAV_LINKS.map((link) => (
              <li key={link}>
                <a
                  href="#"
      ...
      <div className="flex items-center gap-5">
        ...
        <div className="relative cursor-pointer">
          <i className="fas fa-bell text-[20px] text-white" />
          <span className="absolute -top-1 -right-2 bg-[#E50914] text-white text-[10px] font-bold rounded-full w-4 h-4 flex justify-center items-center">
            42
          </span>
        </div>
        ...
        <Link to="/" className="flex items-center gap-1 no-underline text-[#e5e5e5]">
          <img
            ...
            alt="Perfil ativo"
          />
          <i className="fas fa-caret-down text-[14px]" />
        </Link>
      </div>
    </header>
    )
